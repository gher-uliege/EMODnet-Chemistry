...
monthlist = [2,5,8,11]
dateslist = MergingClim.create_date_list(yearrange, monthlist)
...
dmin, dmax =  MergingClim.get_closer_depth([-10., -5., 7., 20], 1.);
@test dmin == -5.
@test dmax == 7.

# Test when outside the depth interval
dmin, dmax =  MergingClim.get_closer_depth([-10., -5., 7., 20], 100.);
@test dmin == nothing
@test dmax == nothing

# Testing the linear interp
w1, w2 = MergingClim.get_depth_weights(15., 10., 30.)
a = [[1 2]; [3 4]]
b = [[5 3]; [3 4]]
ab = w1 * a + w2 * b
...
w1, w2 = MergingClim.get_depth_weights(-10., -20., 0.)
@test w1 == 0.5
@test w2 == 0.5
w1, w2 = MergingClim.get_depth_weights(-5., -20., 0.)
@test w1 == 0.75
@test w2 == 0.25
w1, w2 = MergingClim.get_depth_weights(100., 125., 150.)
@test w1 == nothing
@test w2 == nothing
...
depthinterp = 7.5;
f4interp = MergingClim.depth_interp(depthinterp, depthregion, f4)
...
indmin, indmax = MergingClim.get_depth_indices(6.6, depthgrid)
@test indmin == 2
@test indmax == 3
...
loninterp, latinterp, finterp, indlon, indlat = MergingClim.interp_horiz(lon, lat, f2, longrid, latgrid);
longrid2 = collect(16.5:0.1:17)
loninterp2, latinterp2, finterp2 = MergingClim.interp_horiz(lon, lat, f2, longrid2, latgrid);
